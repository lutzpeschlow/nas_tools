...
func PrepareControl(obj *objects.Control, osName string) error {
	// manage and process actions
	...
	for actionName, enabled := range obj.Enable {
		...
		// create map with key as string but with flexible values - interface
		...
		// merge action‑spezifische Parameter
		...
		if val, ok := actionParams["output_file"]; ok {
			obj.OutputFile = val.(string)
		}
		if val, ok := actionParams["output_dir"]; ok {
			obj.OutputDir = val.(string)
		}
		if val, ok := actionParams["option_01"]; ok {
			obj.Option01 = val.(string)
		}
		if val, ok := actionParams["input_01"]; ok {
